...
#define kRSSURL @"http://www.zapquebec.org/feed/"
#define kRSSURL2 @"http://twitter.com/statuses/user_timeline/65678761.rss"
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	if(!isOffline) {
		...
		CGFloat height = 20.0f + [text sizeWithFont:[UIFont systemFontOfSize:15] 
								  constrainedToSize:CGSizeMake(280, 2000) 
									  lineBreakMode:UILineBreakModeWordWrap].height;
		return MAX(height, 44.0f);	
	...
	if(!isOffline) {
		...
		if([Switcher selectedSegmentIndex]==1)
		{
			...
			//	DetailLabel.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
				...
				Image = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon.png"]] autorelease];
				//CGRect imageFrame = Image.frame;
				CGRect imageFrame=CGRectMake(2, 2, 40, 40);
			...
			DetailLabel.text =  [[[twitterStories objectAtIndex: storyIndex] objectForKey: @"title"] substringWithRange:NSMakeRange(10, [[[twitterStories objectAtIndex: storyIndex] objectForKey: @"title"] length]-10)];
