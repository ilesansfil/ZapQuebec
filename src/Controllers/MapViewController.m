...
- (void)fetchHotspots {
	...
	NSString *urlString = @"http://auth.zapquebec.org/hotspot_status.php?format=XML";
...
- (void)addHotspots {
	...
	for (Hotspot *hotspot in [Hotspot findAll]) {
		...
		if([[[hotspot name] substringWithRange:NSMakeRange(0,6)] isEqualToString:@"ZAP - "])
		{
			NSString *nameShort=[[hotspot name] substringWithRange:NSMakeRange(6, [[hotspot name] length]-6)];
			annotation.title = nameShort;
		}else
		{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	{
		NSString *nameShort=[[hotspot name] substringWithRange:NSMakeRange(6, [[hotspot name] length]-6)];
		if([nameShort length]>27)
		{
			cell.textLabel.text=[nameShort substringWithRange:NSMakeRange(0,26)];
		}else {
			cell.textLabel.text=nameShort;
		}
