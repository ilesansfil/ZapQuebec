...
- (IBAction)visitZAPQuebec {
	UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Visit the ZAP Quebec website?", @"")
															 delegate:self 
													cancelButtonTitle:NSLocalizedString(@"No", @"") 
											   destructiveButtonTitle:nil 
													otherButtonTitles:NSLocalizedString(@"Yes", @""), nil];
	actionSheet.actionSheetStyle = UIActionSheetStyleDefault;
	actionSheet.tag = kActionSheetZAPQuebec;
	[actionSheet showInView:self.tabBarController.view];
	[actionSheet release];
}
...
- (IBAction)visitApache {
	UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Visit the ZAP Quebec website Apache 2.0 license?", @"")
															 delegate:self 
													cancelButtonTitle:NSLocalizedString(@"No", @"") 
											   destructiveButtonTitle:nil 
													otherButtonTitles:NSLocalizedString(@"Yes", @""), nil];
...
- (IBAction)callIleSansFil {
	UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Call ZAP Quebec?", @"")
																				delegate:self 
																	cancelButtonTitle:NSLocalizedString(@"No", @"") 
															 destructiveButtonTitle:nil 
																	otherButtonTitles:NSLocalizedString(@"Yes", @""), nil];
...
- (IBAction)emailIleSansFil {
	UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Send an email to ZAP Quebec?", @"")
																				delegate:self 
																	cancelButtonTitle:NSLocalizedString(@"No", @"") 
															 destructiveButtonTitle:nil 
																	otherButtonTitles:NSLocalizedString(@"Yes", @""), nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	switch (actionSheet.tag) {
		case kActionSheetVisitIleSansFil:
			...
			break;
		case kActionSheetZAPQuebec:
			if (buttonIndex == 0) {
				[[UIApplication sharedApplication] openURL:[NSURL URLWithString: @"http://www.zapquebec.org"]];
			}
			break;
		...
		case kActionSheetCallIleSansFil:
			if (buttonIndex == 0) {
				[[UIApplication sharedApplication] openURL:[NSURL URLWithString: @"tel:+14182638020"]];
			}
			break;
		case kActionSheetEmailIleSansFil:
			if (buttonIndex == 0) {
				[[UIApplication sharedApplication] openURL:[NSURL URLWithString: @"mailto:info@zapquebec.org"]];
			}
			break;
		default:
			break;
